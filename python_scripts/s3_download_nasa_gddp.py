...
def sel_summer(ds):
    ...
    print(f'preprocessing for {season_to_study}')
...
if __name__=="__main__":
    ...
    for season_to_study in ['JJA', 'SON', 'DJF', 'MAM']:
        for model_to_search in ['EC-Earth3']:
            for variable_to_search in ['tasmax', 'huss', 'hurs']:
                print(variable_to_search)
                for experiment_to_search in ['historical', 'ssp585', 'ssp245', 'ssp370']:
                    ...
                    available_files_orig = s3.ls(f'{s3_path}')

                    available_files = [file for file in available_files_orig if '.1.nc' not in file]

                    if len(available_files)!=len(available_files_orig):
                        print('Have filtered out multiple versions')
                        ...
                        if not os.path.isdir(zarr_filename):
                            ...
                            if not os.path.isdir(zarr_filename):
                                try:
                                    ...
                                    newds = newds.convert_calendar('noleap')
                                    newds.to_zarr(zarr_filename, consolidated=True, mode='w')                                    
                                    # newds = newds.chunk({'time':newds['time'].shape[0]})
                                    # pdb.set_trace()
                                    # newds.to_zarr(zarr_filename, consolidated=True, mode='w')
